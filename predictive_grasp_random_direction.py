...
# ---------- marker helpers ----------
def _iter_scene_candidates(uw):
    direct_names = ["scene", "_scene"]
    nested_names = ["sub_scenes", "_sub_scenes", "scenes"]
    seen = set()
...
class DriftPickCubeAxisYawSyncBottomRefZLockEnv(PickCubeEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        offset_x = get_scalar("offset_x", get_scalar("x_offset", 0.0))
        offset_y = get_scalar("offset_y", get_scalar("y_offset", 0.0))
        vx = get_scalar("vx", 0.0)
        vy = get_scalar("vy", get_scalar("drift_speed", 0.0))
...
def motion_frame_axes(vx: float, vy: float):
    """Return unit axes along and perpendicular to the target planar velocity."""
...
def control_xyz_motion_frame(
    ...
    """XY control in target-motion coordinates, Z control in world coordinates."""
    pos_err = (ref_pos - cur_pos).astype(np.float32)
    vel_err = (ref_vel - cur_vel).astype(np.float32)

    along = np.asarray(along_xy, dtype=np.float64)
    perp = np.asarray(perp_xy, dtype=np.float64)
    pos_xy = pos_err[:2].astype(np.float64)
    vel_xy = vel_err[:2].astype(np.float64)
    ref_v_xy = ref_vel[:2].astype(np.float64)

    u_along = (
        float(kp_xyz[1]) * float(np.dot(pos_xy, along))
        + float(kv_xyz[1]) * float(np.dot(vel_xy, along))
        + float(ff_xyz[1]) * float(np.dot(ref_v_xy, along))
    )
    u_perp = (
        float(kp_xyz[0]) * float(np.dot(pos_xy, perp))
        + float(kv_xyz[0]) * float(np.dot(vel_xy, perp))
        + float(ff_xyz[0]) * float(np.dot(ref_v_xy, perp))
    )
    u_xy = u_along * along + u_perp * perp

    u = np.zeros(3, dtype=np.float32)
    u[:2] = u_xy.astype(np.float32)
    u[2] = float(kp_xyz[2]) * float(pos_err[2]) + float(kv_xyz[2]) * float(vel_err[2]) + float(ff_xyz[2]) * float(ref_vel[2])
    return np.clip(u, -max_cmd, max_cmd).astype(np.float32), pos_err, vel_err


# ---------- target-motion velocity-only controller ----------
def control_along_velocity_only(
    target_v_along: float,
    cur_v_along: float,
    dt: float,
    vy_int: float,
    ...
    """Control target-motion-axis velocity only.

    This intentionally does NOT use position error. During dynamic intercept
    phases, the goal is to verify that the execution layer can make the gripper
    velocity along the target-motion axis match the predicted object velocity.
    Position timing can then be tuned separately by changing hit_time /
    launch_time / wait_pos.
    """
    vel_err = float(target_v_along - cur_v_along)
    ...
    # Reuse the empirical y-axis conversion for the target-motion axis.
    action_ff = float(target_v_along) / max(1e-6, float(ee_v_per_action_y))
    action_fb = float(kv_action) * vel_err
    action_i = float(ki_action) * vy_int

    action_along = action_ff + action_fb + action_i
    action_along = float(np.clip(action_along, -float(action_clip), +float(action_clip)))
    return action_along, vy_int, vel_err, action_ff, action_fb, action_i
...
def main():
    ...
    ap.add_argument("--drift_speed", type=float, default=0.03, help="Planar target speed magnitude.")
    ap.add_argument("--motion_angle_deg", type=float, default=90.0, help="Target planar motion direction in degrees: 0=+X, 90=+Y.")
    ap.add_argument("--approach_lead_time", type=float, default=15.0, help="Seconds before the target reaches the grasp-region center.")
    ap.add_argument("--random_direction", action=argparse.BooleanOptionalAction, default=False)
    ap.add_argument("--y_offset", type=float, default=None, help="Legacy +Y offset override; mainly for reproducing old fixed-direction runs.")
    ...
    motion_angle_deg = float(rng.uniform(0.0, 360.0)) if args.random_direction else float(args.motion_angle_deg)
    theta = math.radians(motion_angle_deg)
    target_vx = float(args.drift_speed) * math.cos(theta)
    target_vy = float(args.drift_speed) * math.sin(theta)
    if args.y_offset is not None:
        # Backward-compatible path for old +Y baseline commands.
        offset_x = 0.0
        offset_y = float(args.y_offset)
    else:
        offset_x = -target_vx * float(args.approach_lead_time)
        offset_y = -target_vy * float(args.approach_lead_time)
    print(f"[ablation] variant={args.variant} method_name={args.method_name}")
    print(
        f"[motion] angle={motion_angle_deg:.3f} deg random={int(args.random_direction)} "
        f"v=({target_vx:+.4f},{target_vy:+.4f}) offset=({offset_x:+.4f},{offset_y:+.4f}) "
        f"lead_time={args.approach_lead_time:.3f}"
    )
    ...
    print(
        f"[init] drift_speed={args.drift_speed:.3f} motion_angle_deg={motion_angle_deg:.3f} "
        f"vx={target_vx:+.3f} vy={target_vy:+.3f} offset=({offset_x:+.3f},{offset_y:+.3f}) "
        f"spin_speed={args.spin_speed:.3f} cube_z={args.cube_z:.3f} "
    ...
    print(
        ...
        f"motion_angle_deg={motion_angle_deg:+.3f} log_csv={args.log_csv or 'None'} "
    ...
    for t in range(args.steps):
        ...
        if phase in ("observe", "go_wait", "wait_hold", "launch", "descend", "close", "post_grasp"):
            ...
            frame_vx = float(plan.target_vel[0]) if plan is not None else float(target_vx)
            frame_vy = float(plan.target_vel[1]) if plan is not None else float(target_vy)
            motion_along, motion_perp = motion_frame_axes(frame_vx, frame_vy)
            if phase in ("launch", "descend", "close"):
                u_xyz, pos_err_vec, vel_err_vec = control_xyz_motion_frame(
                    ref_pos=ref_pos,
                    ref_vel=ref_vel,
            ...
            if phase in ("launch", "descend"):
                along_action_scale = float(args.launch_y_action_scale if phase == "launch" else args.descend_y_action_scale)
                if phase == "descend":
                    axis_scale[2] *= args.descend_z_action_scale
                u_xy = u_xyz[:2].astype(np.float64)
                u_along = float(np.dot(u_xy, motion_along))
                u_perp = float(np.dot(u_xy, motion_perp))
                action[:2] = (
                    float(axis_scale[0]) * along_action_scale * u_along * motion_along
                    + float(axis_scale[0]) * u_perp * motion_perp
                ).astype(np.float32)
                action[2] = float(u_xyz[2]) * float(axis_scale[2])
            else:
            ...
            # In dynamic intercept phases, target-motion-axis velocity is
            # controlled without position error. The perpendicular planar
            # component and z keep the position/velocity controller above.
            if args.vy_vel_only and phase in ("launch", "descend", "close"):
                target_v_along = float(np.dot(ref_vel[:2].astype(np.float64), motion_along))
                cur_v_along = float(np.dot(ee_v[:2].astype(np.float64), motion_along))
                ...
                action_perp = float(np.dot(action[:2].astype(np.float64), motion_perp))
                action[:2] = (float(action_along) * motion_along + action_perp * motion_perp).astype(np.float32)
                vy_only_active = True
        ...
        if args.log_csv and (t % max(1, int(args.log_every)) == 0 or t == args.steps - 1):
            ...
            log_rows.append({
                ...
                "motion_angle_deg": _safe_float(motion_angle_deg),
                "random_direction": int(args.random_direction),
                "target_vx_cmd": _safe_float(target_vx),
                "target_vy_cmd": _safe_float(target_vy),
                "offset_x_cmd": _safe_float(offset_x),
                "offset_y_cmd": _safe_float(offset_y),
                "spin_speed": _safe_float(args.spin_speed),
                "y_offset": _safe_float(args.y_offset) if args.y_offset is not None else float("nan"),
    ...
    if args.summary_csv:
        ...
        summary = {
            ...
            "offset_x_cmd": _safe_float(offset_x),
            "offset_y_cmd": _safe_float(offset_y),
            "approach_lead_time": _safe_float(args.approach_lead_time),
            "spin_speed": _safe_float(args.spin_speed),
            "y_offset": _safe_float(args.y_offset) if args.y_offset is not None else float("nan"),
